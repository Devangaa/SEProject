...
<div class="bg-white p-8 md:p-10 rounded-[2.5rem] shadow-sm border border-gray-100 w-full max-w-md mx-auto" x-data="forgotPasswordHandler()">
    ...
    <div x-show="step === 1" x-transition>
        <div class="mb-5">
            ...
            <input type="email" 
                   x-model="email"
                   class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent focus:outline-none transition placeholder-gray-300"
                   placeholder="Masukkan email anda">
        </div>

        <div class="flex flex-col items-end mb-6">
            <a href="{{ route('login') }}" class="text-[11px] text-amber-700 font-bold hover:underline mb-2">Kembali ke login</a>
    ...
    <div x-show="step === 2" x-cloak x-transition.opacity>
        ...
        <div class="mb-6">
            ...
            <input type="text" 
                   x-model="otp"
                   maxlength="6"
                   class="w-full px-4 py-4 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent focus:outline-none transition text-center tracking-[0.5em] font-bold text-2xl"
                   placeholder="000000">
        </div>
        ...
        <button @click="verifyOtp" 
                :disabled="loading"
                class="w-full bg-amber-700 hover:bg-amber-800 text-white font-bold py-3.5 rounded-xl transition shadow-lg shadow-amber-100">
            Verifikasi Kode
        </button>
        
        <button @click="step = 1; error = ''" class="w-full mt-4 text-[11px] text-gray-400 font-bold hover:text-amber-700 transition">Ganti Email</button>
    </div>
</div>
