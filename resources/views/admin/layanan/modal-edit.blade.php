            ...
            <form :action="editUrl" method="POST" enctype="multipart/form-data" class="p-8"
                x-data="{ 
                    ...
                    handleFileSelect(event) {
                        ...
                        files.forEach(file => {
                            ...
                            if ((editData.foto_layanan.length - this.removedImages.length + this.newPhotos.length) < 4) {
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2 space-y-4">
                        <label class="text-sm font-bold text-gray-700 ml-1">Manajemen Foto Layanan (Maks 4)</label>
                        
                        <div class="grid grid-cols-4 gap-4" x-show="editData.foto_layanan && editData.foto_layanan.length > 0">
                            <template x-for="(img, index) in editData.foto_layanan" :key="'old-'+index">
                                <div class="relative group aspect-square rounded-2xl overflow-hidden border-2 transition-all"
                                    ...
                                    <img :src="'/uploads/layanan/' + img" class="w-full h-full object-cover">
                        ...
                        <div class="mt-4 space-y-3">
                            ...
                            <div class="grid grid-cols-4 gap-4">
                                ...
                                <label x-show="(editData.foto_layanan.length - removedImages.length + newPhotos.length) < 4" 
                                    ...
                                    <input type="file" x-ref="fileInput" name="foto_layanan[]" multiple accept="image/*" class="hidden" @change="handleFileSelect">
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="space-y-2 text-left">
                        ...
                        <input type="text" name="nama_layanan" x-model="editData.nama_layanan" required class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-green-500 outline-none text-sm">
                    </div>
                    ...
                    <div class="md:col-span-2 space-y-2 text-left">
                        ...
                        <textarea name="deskripsi" rows="4" x-model="editData.deskripsi" class="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-green-500 outline-none text-sm resize-none"></textarea>
                    </div>
                </div>

                <div class="mt-10 flex justify-end gap-4">
                    <button type="button" @click="showEditModal = false" class="px-8 py-4 bg-gray-100 text-gray-600 font-bold rounded-2xl hover:bg-gray-200 transition-all">Batal</button>
                    <button type="submit" 
