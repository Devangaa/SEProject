        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div data-aos="fade-right">
                ...
                <h1 class="text-4xl font-extrabold text-gray-900 leading-tight">
                    Pilih Produk <span class="text-amber-700">Favorit</span> Anda
                </h1>
            ...
            <form id="product-category-form" action="{{ route('produk.index') }}" method="GET" class="flex flex-col md:flex-row flex-wrap gap-4 items-start md:items-center">
                <div class="relative w-full md:flex-1 md:min-w-[300px]">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari benih, selada, atau nutrisi"
                        class="w-full pl-12 pr-12 py-3.5 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-amber-500 transition outline-none text-sm">
                ...
                <div x-data="{ openCategory: false }" class="relative w-full md:w-auto">
                    <button type="button" @click="openCategory = !openCategory" class="w-full md:w-auto bg-gray-50 border-none rounded-xl px-6 py-3.5 text-sm font-bold text-gray-600 outline-none focus:ring-2 focus:ring-amber-500 transition flex items-center justify-between gap-3 hover:bg-gray-100">
                    ...
                    >
                        <button type="button" @click="openCategory = false; document.querySelector('#product-category-form input[name=category]').value = ''; setTimeout(() => document.querySelector('#product-category-form').submit(), 50)" class="w-full text-left px-6 py-3 text-sm font-bold text-gray-600 hover:bg-amber-50 hover:text-amber-700 transition {{ !request('category') ? 'bg-amber-50 text-amber-700' : '' }}">
                            Semua Kategori
                        </button>
                        @foreach($categories as $cat)
                            <button type="button" @click="openCategory = false; document.querySelector('#product-category-form input[name=category]').value = '{{ $cat }}'; setTimeout(() => document.querySelector('#product-category-form').submit(), 50)" class="w-full text-left px-6 py-3 text-sm font-bold text-gray-600 hover:bg-amber-50 hover:text-amber-700 transition {{ request('category') == $cat ? 'bg-amber-50 text-amber-700' : '' }}">
                                {{ $cat }}
                            </button>
                        @endforeach
                    </div>
        ...
        @if($products->hasPages())
            <div class="mt-12 flex flex-col md:flex-row justify-between items-center gap-4" data-aos="fade-up">
                ...
                <div class="flex gap-1">
                    ...
                    @else
                        <a href="{{ $products->appends(request()->query())->previousPageUrl() }}" class="px-4 py-2 bg-white border border-gray-100 text-gray-600 text-xs font-bold rounded-xl hover:bg-amber-700 hover:text-white hover:border-amber-700 transition-all duration-300">
                            &lt;
                        </a>
                    @endif
                    ...
                    @if ($products->hasMorePages())
                        <a href="{{ $products->appends(request()->query())->nextPageUrl() }}" class="px-4 py-2 bg-white border border-gray-100 text-gray-600 text-xs font-bold rounded-xl hover:bg-amber-700 hover:text-white hover:border-amber-700 transition-all duration-300">
                            &gt;
                        </a>
                    @else
                        <span class="px-4 py-2 bg-white border border-gray-100 text-gray-300 text-xs font-bold rounded-xl cursor-not-allowed">
